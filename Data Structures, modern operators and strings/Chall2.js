...
for (let [index, player] of game.scored.entries()) console.log(`Goal ${index + 1}: ${player}`);
...
avarageOdd /= oddValues.length;
console.log(avarageOdd);

for (let [key, value] of Object.entries(game.odds)) {
    // const teamStr = key === `x`? `draw` : `${game[key]}'s victory`;
    console.log(`Odd of ${game?.[key] || "draw"} is ${value}`);
}
